...
$lConf = $conf;
debug($lConf,"skrypt DIBS:lconf");//MKL
$localTemplateCode = $this->cObj->fileResource($lConf[templateFile] ? $lConf[templateFile] : "EXT:mkl_products/pi/payment_DIBS_template_uk.tmpl");		// Fetches the DIBS template file
	...
	default:
		if ($lConf["relayURL"])	{
			...
			$content=$this->getBasket("###DIBS_REDIRECT_TEMPLATE###",$localTemplateCode, $markerArray);		// This not only gets the output but also calculates the basket total, so it's NECESSARY!
                                                debug($markerArray["###REDIRECT_URL###"],"DIBS.php:marker");//MKL
                                                debug($content,"DIBS.php:content");//MKL
		} else {
